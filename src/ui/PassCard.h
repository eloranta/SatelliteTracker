...
// marker (updated every 1s via tick()). Owns its own propagator so
// per-second updates don't need to go back through PassGridWidget's
// off-thread recompute cycle. PassGridWidget rebuilds its whole card set
// each ~30s cycle rather than reusing cards, so a given PassCard instance
// only ever receives one setPassResult() call.
//
